...
The process is simple and common to many organizations.
...
\begin{table}[ht]
  \begin{tabu}{| l | X |}
    ...
    \hline
  \end{tabu}
  \caption{Requirements for a synchronous communication in reactive IS}\label{tab:requirements}
\end{table}
...
The description of commands matches the the problem closely and it is also elaborated upon some more.
\cite[107--108]{stopford_designing_2018} propose that in order to follow the \gls[]{swp}, a two topic approach---one for domain events and one for commands---can be implemented.
...
Furthermore, this section shows how the concept can be integrated into the \gls[]{is} architecture presented in section \ref{sec:application}.
...
\emph{Service} A publishes a command with a correlation ID and a direction to its own reply top to the command topic of \emph{Service B}.
Service B processes the command and them publishes a reply message to \emph{Service A}'s reply topic as defined in the command.
...
Beyond beyond the approach discussed above, here \emph{Service A} also has a connected persistent storage where it writes meta-information to keep track of the commands it has sent out.
Table \ref{tab:command-datamodel} presents the schema for such storage.
Through this storage, \emph{Service A} keeps the information about the commands that it has sent and can resume consuming replies once it has recovered.
This allows the concept to fulfill requirement N1.

To address the requirement F2 as well, the concept features a worker thread that runs in parallel to the regular execution threads of \emph{Service A} and periodically checks the storage for timed out commands.
The next section clarifies this concept further by integrating it into the example architecture from section \ref{sec:problem}.
...
\begin{listing}[h]
  ...
  \caption{An example \texttt{CreateApprovalRequest} command}\label{lst:create-approval-request}
\end{listing}
...
Therefore, the goal of this work was defined as creating a concept for implementing synchronous communication in a reactive \gls[]{is} and method of achieving it was based on the \gls[]{dsr} roadmap of \cite[]{alturki_design_2011}
...
The result of this work is a details examination of the problem space around synchronous communication in reactive systems as well as an exploration of professional literature regarding that topic.
The concept which was synthesized from that fulfills the identified requirements and appears promising.
However, evaluating it artificially like that is only the first step of testing its quality.
